        ...
        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
          {transactions.length > 0 ? (
            <div>
              ...
              <p className='mt-1 text-base text-slate-600'>Latest income and expense transactions in one place.</p>
            </div>
          ) : (
            <div>
              <p className='text-sm font-semibold uppercase tracking-[0.2em] text-slate-500'>Transactions</p>
              <p className='mt-1 text-base text-slate-600'>View and manage your transactions.</p>
            </div>
          )}
      ...
      {filteredTransactions.length === 0 ? (
        <div className='p-8 text-center text-slate-500'>
          ...
          <p className='mt-2 text-sm'>
            {filterBy === 'all' ? 'Belum ada transaksi untuk periode ini.' : `Belum ada transaksi ${filterBy === 'income' ? 'pemasukan' : 'pengeluaran'} untuk periode ini.`}
          </p>
        </div>
      ) : (
        <div className='overflow-x-auto'>
          <table className='min-w-full border-separate border-spacing-0'>
            <thead className='bg-slate-100 text-slate-600'>
              <tr>
                <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.15em]'>Title</th>
                <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.15em]'>Type</th>
                <th className='px-6 py-4 text-right text-xs font-semibold uppercase tracking-[0.15em]'>Amount</th>
                <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.15em]'>Date</th>
                <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.15em]'>Created At</th>
                <th className='px-6 py-4 text-center text-xs font-semibold uppercase tracking-[0.15em]'>Action</th>
              </tr>
            </thead>
